...
| **Gateway Cron** | `alpaca_setup_gateway_cron` creates OpenClaw Gateway jobs that wake the agent and run `alpaca_cron_tick` |
...
- `alpaca_setup_gateway_cron` — Create OpenClaw Gateway cron jobs for automated wakeups
